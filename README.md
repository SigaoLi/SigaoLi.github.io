...
- **Built-in AI layer** — a floating chat assistant (fronted by Zoe) on every page plus a personal MCP server, both fed by a build-time knowledge pack generated from the same sources as the pages (see below)
...
worker/               # Cloudflare Worker: /chat (SSE) + /mcp (MCP server)
