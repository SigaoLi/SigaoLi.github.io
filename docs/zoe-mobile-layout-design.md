...
3. **极窄高时面板铺满（`.tight`，2026-09-03 已做）**。320×568 @200% 缩放时可用高只剩 284px，面板连自己都装不下。改为：可用高（已扣掉键盘）低于 420px 时，面板四边留白收到 6px、圆角减半、宽度铺满。
...
4. **`ball` / `stretch` 画幅溢出 —— 判定为不修**。逐帧实测（桌面 s=1）：`ball` 越界 66px、占 13% 帧，被裁的是**球**不是猫（猫稳定在画幅 387–678，右缘 `innerWidth−50` 安全）；`stretch` 越界 16px、占 8% 帧。全部 18 段 × 两档尺寸的完整审计确认**只有这两段、只在右缘、且只有桌面真会播**（手机端两者都被 `Z.interactive` 挡住），底缘无溢出。
